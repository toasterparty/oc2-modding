    ...
    public static class OC2Config
    {
        ...
        public class ReceiveJoinRequestMessage_Patch
        {
            ...
            )
            {
                ...
                OC2Modding.Log.LogInfo("Successfully paired with another modded client of the same mod version");
        ...
        private static class UpdateJoiningLobby_Patch
        {
            private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
            {
                ...
                if (!JsonMode)
                {
                    return code; // just some harmless QoL
                }
                ...
                {
                    if (
                        code[i].opcode == OpCodes.Call &&          // get CodeVersion
                        code[i + 1].opcode == OpCodes.Ldc_I4_S &&  // Load Literal: 32 (bits)
                        (sbyte)(code[i + 1].operand) == 32 &&
                        code[i + 2].opcode == OpCodes.Callvirt     // Write to stream
                        )
                    {
                        injectionIndex = i;
                        break;
